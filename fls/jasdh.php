<?php
  		...
  		mysql_select_db("mojipodaci") or die(mysql_error()); 
                ...
                if (isset($_POST["spremi"]))
                    { 
                    if( !mysql_query("INSERT INTO obr_81(`jasdkj`, `jasd`) VALUES ('$_POST[tb0]', '$_POST[rb1]' ) ") )
                        { echo mysql_error(); }
...
<b><p id="naz">jasdh</p></b>

jasdkj: <input type="text" class="textbox" name="tb0"><br>jasd: <br>asdj <input type="radio" name="rb1" value="asdj">  <br>asdj <input type="radio" name="rb1" value="asdj">  <br><input type="submit" name="spremi" value="Unos" class="button"> </form></div><div class="prewrite" id="okv2">  <b>Prikaz podataka:</b> <br><br><table class='gridtable' align='center'>
					<?php $sql_q = 'select * from information_schema.columns where table_schema = "mojipodaci" and table_name = "obr_81" '; 
						$reee = mysql_query($sql_q);
						while ($reed = mysql_fetch_row($reee))
						{ echo '<th>'.$reed[3] .'</th>'; } ?><?php if(!($res=mysql_query('SELECT * FROM obr_81 '))) 
					{ echo'Nema sadrzaja!'; }
...
jasdkj: <input type="text" class="textbox" name="tb0"><br>jasd: <br>asdj <input type="radio" name="rb1" value="asdj">  <br>asdj <input type="radio" name="rb1" value="asdj">  <br></textarea></div>